<template>
    <div>
        <div :class="['relative mt-2 rounded-md shadow-sm', className]">
            <label
            ...
            <div
                @click="show = !show"
                class="pointer absolute inset-y-0 right-0 flex items-center pr-3"
            >
                <EyeIcon
                    v-if="!show"
...
const css = computed(() => {
    ...
    return "block w-full rounded-md border-0 py-1.5 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary-light sm:text-sm sm:leading-6";
});
</script>
